...
class KDTree:
    ...
    def closest(self, point, sidx=si.StorageIndex()):
        ...
        # Base case: reached leaf node, return all elements in this bbox.
        if "elements" in self.storage[sidx.storage()]:
            return self.storage[sidx.storage()]["elements"]

        # Otherwise, check axis and fetch partition value.
        axis = self.storage[sidx.storage()]["axis"]
        partition = self.storage[sidx.storage()]["partition"]

        # Check if the point lies to the left or right of the partition
        # and recursively search again.
        if point[axis] < partition:
            values = self.closest(point, sidx.left())
        else:
            values = self.closest(point, sidx.right())
