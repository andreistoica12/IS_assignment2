...
class BoundingBox:
    ...
    def compute_quads(self):
        """
        Returns  a list of bboxes that form the quad children of the current bbox.
        """
        ...
        # Lower-left quad
        quad_list.append(self.from_matrix(np.array([
            [self.data[0, 0], centroid[0]],  # x ranges
            [self.data[1, 0], centroid[1]]   # y ranges
        ])))

        # Upper-left quad
        quad_list.append(self.from_matrix(np.array([
            [self.data[0, 0], centroid[0]],
            [centroid[1], self.data[1, 1]]
        ])))

        # Lower-right quad
        quad_list.append(self.from_matrix(np.array([
            [centroid[0], self.data[0, 1]],
            [self.data[1, 0], centroid[1]]
        ])))
